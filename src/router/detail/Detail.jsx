        ...
        <div className='bg-white'>
            <div className="container flex flex-col gap-[42px]">
                <div className='flex flex-col gap-[40px]'>
                    ...
                    <p>
                        <NavLink to="/product">Products</NavLink> / Gaming Headsets & Audio / <span className='font-bold'>{data?.name || 'Loading...'}</span>
                    </p>
                </div>
                <div className='flex mb-[50px] justify-between'>
                    <div className='w-[700px] flex flex-col gap-[145px]'>
                        ...
                        <div className='flex gap-[20px] ml-[-40px] cursor-pointer'>
                            {data?.image_url && [...Array(5)].map((_, index) => (
                                <img key={index} className='w-[15%] justify-center border p-[10px]' src={data.image_url} alt="" />
                            ))}
                        </div>
                    </div>
                    <div>
                        <div className='flex flex-col gap-[18px]'>
                            ...
                            <div className='flex flex-col'>
                                ...
                                <div className='flex gap-[12px] mt-[32px] mb-[12px]'>
                                    {data?.color_options.map((color, index) => (
                                        <p key={index} className='w-[30px] h-[30px] border cursor-pointer mt-[-20px] rounded-[50%]' style={{ backgroundColor: color }}></p>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className='flex items-center gap-[17px]'>
                                <button
                                    ...
                                    disabled={!!addedItems[data?.id]}
                                    className={`flex gap-[10px] items-center mt-[10px] text-white text-[22px] font-medium rounded-[10px] px-[33px] py-[16px] ${addedItems[data?.id] ? 'bg-gray-400 opacity-80 cursor-not-allowed' : 'bg-[rgb(11,164,45)]'}`}
                                >
                                    <IoCartOutline />
                                    {addedItems[data?.id] ? 'Added' : 'Add to Cart'}
                                </button>
